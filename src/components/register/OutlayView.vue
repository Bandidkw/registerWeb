<template>
    <div class="containerregister">
        ...
        <div class="logoregistor">
            <img src="../../assets/image/logo_register2.png" alt="Image" style="width: 80%" @click="$router.push('/')" />
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        async registor() {
            ...
            this.iden = this.iden.replace("-", "");
            const data = {
                outlay_name: this.name,
                outlay_phone: this.tel,
                outlay_iden: this.iden,
                outlay_email: this.email,
                outlay_username: this.username,
                outlay_password: this.password,
                outlay_address: this.address,
                outlay_subdistrict: this.tambon.name_th,
                outlay_district: this.amphure.name_th,
                outlay_province: this.province.name_th,
                outlay_postcode: this.postcode,
            };
            await axios.post(`${process.env.VUE_APP_SHOP}/register/outlay`,
                data,
            ).then(() => {
                this.$toast.add({
                    severity: "success",
